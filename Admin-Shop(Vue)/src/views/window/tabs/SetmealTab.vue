...
const getSetmealList = async () => {
    loading.value = true
    try {
        const { data: res } = await getSetmealPageListAPI({
            page: pageData.page,
...
// 修改套餐状态
const changeStatus = async (row: Setmeal) => {
    try {
...
const deleteBatch = (row?: Setmeal) => {
    ElMessageBox.confirm('该操作会永久删除套餐，是否继续？', '警告', {
...
<template>
    <el-card shadow="never">
        ...
        <div class="filter-bar">
            <el-input v-model="pageData.name" placeholder="套餐名称" clearable style="width: 160px" />
            ...
            <el-button type="primary" @click="getSetmealList">查询</el-button>
        ...
        <!-- 套餐列表 -->
        <el-table v-loading="loading" ref="multiTableRef" :data="setmealList" stripe
            ...
            <el-table-column prop="pic" label="图片" width="80" align="center">
                <template #default="{ row }">
                    <img v-if="row.pic" :src="row.pic" class="setmeal-img" />
                    <span v-else>-</span>
                </template>
            </el-table-column>
