...
import { getDishPageListAPI, updateDishStatusAPI, deleteDishesAPI, batchUpdateDishStatusAPI } from '@/api/dish'
...
const handleBatchCommand = (command: string) => {
  if (multiSelection.value.length === 0) {
    ElMessage({ type: 'warning', message: '请先选择要操作的菜品' })
    return
  }
...
const batchUpdateStatus = (status: number) => {
  ...
  ElMessageBox.confirm(
    ...
    .then(async () => {
      const res = await batchUpdateDishStatusAPI(ids, status)
